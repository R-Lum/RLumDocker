...
> [R.Luminescence repositories on Docker
> Hub](https://hub.docker.com/u/rlum/)

> [![Build
> Status](https://travis-ci.org/R-Lum/RLumDocker.svg?branch=master)](https://travis-ci.org/R-Lum/RLumDocker)
...
The base image `rlum/core` builds on top of `rocker/rstudio:latest`
([see the ‘rocker’-project by D. Eddelbuettel and C.
Boettiger](https://github.com/rocker-org/rocker)) with additional
installs of:
...
`rlum/stable` contains the following packages currently available on
CRAN:
...
`rlum/dev` installs the latest development versions of these packages
via `devtools::install_github()` from the following repositories:
...
running an [Apache server with PHP 7.0](https://hub.docker.com/_/php/)
that hosts [DRAC](https://github.com/DRAC-calculator/DRAC-calculator).
Pre-made images are available
[here](https://hub.docker.com/r/rlum/drac/) and can be run using `docker
run -d -p 80:80 rlum/drac:<version>` (replace `<version>` with any of
the tags listed [here](https://hub.docker.com/r/rlum/drac/tags/)). Once
